...
/// On-device job library: one `<id>.shortcast` bundle per finished job under
/// `~/Library/Application Support/Shortcast/Library/`, holding a Codable
...
enum JobLibrary {
    ...
    private static let bundleExt = "shortcast"

    /// `~/Library/Application Support/Shortcast/Library/`, created if needed.
    nonisolated static func root() throws -> URL {
        let base = try FileManager.default.url(
            for: .applicationSupportDirectory, in: .userDomainMask,
            appropriateFor: nil, create: true)
        let dir = base.appendingPathComponent("Shortcast/Library", isDirectory: true)
