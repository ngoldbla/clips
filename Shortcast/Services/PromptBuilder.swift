...
enum PromptBuilder {
    ...
    static func buildPrompt(coach: String, languageOverride: String, styleExamples: String) -> String {
        ...
        if language.isEmpty {
            sections.append("""
            ## Output language
            Write every field in the SAME language that is spoken in the video — \
            detect it from the audio. Do not translate it to English.
            """)
        } else {
            sections.append("""
            ## Output language
            Write every field in this language: \(language). \
            Use it regardless of the language spoken in the video.
            """)
        }
